...
def pgd_attack(
    ...
    for k in range(num_image//batch_size):
        ...
        for step in range(num_steps):
            ...
            bsz = latents.shape[0]
            # Sample a random timestep for each image
            timesteps = torch.randint(600, int(noise_scheduler.config.num_train_timesteps), (bsz,), device=latents.device)
            timesteps = timesteps.long()
            ...
            #advdm_loss = F.mse_loss(model_pred.float(), target.float(), reduction="mean")
            advdm_loss = F.mse_loss(model_pred , target )
            loss = advdm_loss
            #print(mask == 1.)
            # target-shift loss
            if target_tensor is not None:
                ...
                # fused mode
                if mode == 'fused':
                    semantic_loss = F.mse_loss(latents, target_tensor)
                    loss = loss - 1e2 * semantic_loss
            ...
            if step % args.gradient_accumulation_steps == args.gradient_accumulation_steps - 1:
                adv_images = perturbed_image + alpha * perturbed_image.grad.sign()  #pgd attack
...
def main(args):
    ...
    f = [unet, text_encoder]
    # mask is of shape (batch_size, 32, 32, 4)
    #mask = torch.ones_like(target_latent_tensor)
    #print(mask.shape)
    ##mask[:, :,5:50, 5:27] = 1.
    #mask[:, :, 5:40, 5:40] = 1.
    #print_tensor(mask[0][0])
